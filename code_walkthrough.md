...
- **Line 10**: Defines the function `home()`.
- **Line 11**: `render_template`. It opens `templates/index.html` and sends it to the browser. We pass empty `recipes` because we aren't searching for anything yet.
...
- **Line 18**: Calls our helper function `search_recipes(query)` to get a list of recipes.
- **Line 19**: Reloads the page (`index.html`), but this time passes the `recipes` list we found so the HTML can display them.
...
### Helper Function: `search_recipes`
```python
...
- **Line 54**: Loads a *different* HTML page: `view_recipe.html`.
...
## 3. Frontend Explanation (`templates/index.html`)
...
- **Line 70**: `<form>`. This is the interactive part.
    ...
    - `action="/"`: Tells the browser "Send this data to the `/` route in `app.py`".
...
89:     </ul>
```
- **Line 77**: `{% if recipes %}`. This is **Jinja2**. It runs on the server *before* the HTML is sent to the user. It says: "Only show this section if the `recipes` list is not empty."
...
3.  **The Kitchen (Backend/Flask)**: Receives the order. The Chef (`app.py`) looks at the order.
...
3.  **Backend**:
    ...
    - It fills out `view_recipe.html` with those specific details.
